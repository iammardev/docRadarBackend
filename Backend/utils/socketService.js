...
export const initializeSocket = (server) => {
  io = new Server(server, {
    cors: {
      origin: ["http://localhost:3000","http://localhost:5174" , "http://localhost:5173","http://localhost:8080"],
      methods: ["GET", "POST"],
      credentials: true,
    },
  });
...
//         // Find all doctors with matching specialty (removed radius limitation)
//         const nearbyDoctors = await findAllDoctorsBySpecialty(
//           location.coordinates[0], // longitude
//           location.coordinates[1], // latitude
//           specialty
//         );
...
//         if (nearbyDoctors.length === 0) {
//           io.to(socket.id).emit("no_doctors_available", {
//             message: "No doctors available with the selected specialty",
//           });
//           return;
//         }
...
//         // Find all emergency doctors (removed radius limitation)
//         const emergencyDoctors = await findAllEmergencyDoctors(
//           location.coordinates[0], // longitude
//           location.coordinates[1], // latitude
//         );
...
//           location,
//           requestId,
//           doctorIds: emergencyDoctors.map((doctor) => doctor._id.toString()),
//           timestamp: Date.now(),
//         });

//         // Send request to all emergency doctors
//         emergencyDoctors.forEach((doctor) => {
//           const doctorSocketId = doctorSockets.get(doctor._id.toString());
//           if (doctorSocketId) {
//             io.to(doctorSocketId).emit("emergency_search_request", {
//               requestId,
...
//         io.to(socket.id).emit("emergency_doctors_found", {
//           requestId,
//           doctors: emergencyDoctors.map(doctor => ({
//             doctorId: doctor._id,
//             name: doctor.name,
//             specialty: doctor.specialty,
//             hospital: doctor.currentHospital,
...
//             distance: doctor.distance
//           })),
//           message: `Found ${emergencyDoctors.length} emergency doctors`,
//         });
...
// // Find all doctors by specialty (removed radius limitation)
// async function findAllDoctorsBySpecialty(longitude, latitude, specialty) {
//   try {
//     const query = {
//       specialty: specialty,
//       isApproved: true
...
// // Find all emergency doctors (removed radius limitation)
// async function findAllEmergencyDoctors(longitude, latitude) {
//   try {
//     const query = {
//       emergencyAvailability: true,
//       isApproved: true
